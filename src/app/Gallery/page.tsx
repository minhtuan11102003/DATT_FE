...
const GalleryPage = () => {
  ...
  const arrProducts: { id: number, title: string, categoryId: number, img: string }[] = [
    {
      id: 1,
      categoryId: 2,
  ...
  const getProductId = (id:number) =>{
    let arrProductId = arrProducts.find(item=>(
      item.id == id
    ))
                  ...
                  <div className="carousel-inner pr-8" role="listbox">
                    <div className="carousel-item active ">
                      <img
                        src={`${mangImg[dem].img}`}
                        alt="Lỗi ảnh"
                        className={`chuyenDong ${isChuyenDong ? 'animate-in' : 'animate-out'
